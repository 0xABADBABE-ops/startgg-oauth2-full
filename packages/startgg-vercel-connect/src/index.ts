...
export const STARTGG_VERCEL_CONNECT_CONFIG = {
  ...
  scopesRequiringIdentity: ['user.email', 'tournament.manager', 'tournament.reporter'],
...
export type StartGGScope = (typeof STARTGG_VERCEL_CONNECT_CONFIG.scopes)[keyof typeof STARTGG_VERCEL_CONNECT_CONFIG.scopes];

export type StartGGScopeValue = (typeof STARTGG_VERCEL_CONNECT_CONFIG.allScopes)[number];
...
export function isValidStartGGScope(scope: string): scope is StartGGScopeValue {
  return STARTGG_VERCEL_CONNECT_CONFIG.allScopes.includes(scope as StartGGScopeValue);
}
...
export const vercelConnectorConfig = {
  ...
  config: {
    ...
    tokenEndpointAuthMethod: STARTGG_VERCEL_CONNECT_CONFIG.tokenEndpointAuthMethod,
  },
} as const;
