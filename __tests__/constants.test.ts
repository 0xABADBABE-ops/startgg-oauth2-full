import { STARTGG_ENDPOINTS, STARTGG_SCOPES, STARTGG_GQL_AUTH_HEADER, isValidStartGGScope, type StartGGScopeValue } from '../src/constants';

describe('StartGG Constants', () => {
  describe('STARTGG_ENDPOINTS', () => {
    test('authorize endpoint is correct', () => {
      expect(STARTGG_ENDPOINTS.authorize).toBe('https://api.start.gg/oauth/authorize');
    });

    test('token endpoint is correct', () => {
      expect(STARTGG_ENDPOINTS.token).toBe('https://api.start.gg/oauth/token');
    });
