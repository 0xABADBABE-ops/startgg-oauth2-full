...
describe("StartGGOAuth2Handler", () => {
  ...
  test("exchangeToken fails when token_type not Bearer", async () => {
    ...
    const handler = createStartGGAuth2Handler(cfg);
    await expect(
      handler.exchangeToken("code", "verifier", [StartGGScope.USER_IDENTITY]),
  ...
  test("exchangeToken throws ScopeValidationError with details", async () => {
    ...
    const handler = createStartGGAuth2Handler(cfg);
    await expect(
      handler.exchangeToken("code", "verifier", [
        StartGGScope.USER_IDENTITY,
        StartGGScope.USER_EMAIL,
      ]),
  ...
  test("handler uses custom fetchTimeoutMs", async () => {
    ...
    await handler.exchangeToken("code", "verifier", [
      StartGGScope.USER_IDENTITY,
    ]);
  ...
  test("factory defaults to STARTGG_ENDPOINTS", async () => {
    ...
    const callArgs = jest.spyOn(global, "fetch" as any).mock
      .calls[0][0] as string;
    expect(callArgs).toBe("https://api.start.gg/oauth/access_token");
  });
});
