...
async function getSubtleCrypto(): Promise<SubtleCrypto> {
	...
	throw new OAuth2Error(
		"WebCrypto subtle not available; required for PKCE S256",
		"CRYPTO_UNAVAILABLE",
	);
}
...
function base64Encode(bytes: Uint8Array): string {
	...
	if (typeof g.btoa !== "function")
		throw new OAuth2Error(
			"btoa not available for base64 encoding",
			"B64_UNAVAILABLE",
		);
...
function validateScopesOrAssumePrevious(
...
export async function buildAuthorizeUrl(
	...
	if (opts.codeVerifier && opts.codeChallenge) {
		...
		if (expectedChallenge !== opts.codeChallenge) {
			throw new OAuth2Error(
				"Provided codeChallenge does not match codeVerifier",
				"INVALID_PKCE_PAIR",
			);
		}
		...
		throw new OAuth2Error(
			"codeVerifier is required when providing codeChallenge",
			"INVALID_PKCE_PAIR",
		);
	} else {
...
export class StartGGOAuth2Handler implements IOAuth2HandlerWithPKCE {
	...
	async exchangeToken(
		...
		validateScopesOrAssumePrevious(
			tokenResponse.scope,
			expectedScopes.map(String),
		);
	...
	async refreshToken(
		...
		const res = await fetchWithTimeout(this.config.tokenEndpoint, {
			...
			body: formBody(
				Object.fromEntries(
					Object.entries(refreshRequest).filter(([, v]) => v != null),
